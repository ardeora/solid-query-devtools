...
export const DevtoolsPanel: Component<DevtoolsPanelProps> = (props) => {
	...
	const queryCount = createSubscribeToQueryCache((queryCache) => {
		...
		for (const label of queryStatusLabels) {
			res[label] = curr.filter(
				(e) => getQueryStatusLabel(e) === label
			).length;
		}
		return res;
	});
			...
			class={css`
				...
				& .SQD-panel-enter {
					transform: translateY(
						${Number(devtoolsHeightStore.height || 500)}px
					);
				}
			...
			<TransitionGroup name="SQD-panel">
				<Show when={openStore.open === "true"}>
					<aside
						class={styles.panel}
						style={{
							height: `${devtoolsHeightStore.height || 500}px`,
						}}
					>
						<div
						...
						<div class={styles.queriesContainer}>
							...
							>
								...
								>
									...
									<span class={styles.solidQueryLogo}>
										Solid Query v5
									</span>
								</button>
								<QueryStatusCount />
...
export const QueryRow: Component<{ query: Query }> = (props) => {
	...
	const data = createSubscribeToQueryCache((queryCache) => {
		const queryState = queryCache().find({
	...
	const color = createMemo(() =>
		getQueryStatusColor({
			...
			observerCount: data().observers,
			isStale: data().isStale,
		})
	);

	return (
		<Show when={data().queryState}>
			<button
				onClick={() =>
