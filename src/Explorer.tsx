			...
			<Show when={props.subEntryPages.length}>
				...
				>
					...
					<span class={styles.info}>
						{String(props.type).toLowerCase() === "iterable"
							? "(Iterable) "
							: ""}
						{props.subEntries.length}{" "}
						{props.subEntries.length > 1 ? `items` : `item`}
					</span>
				</button>
								...
								{(entries, index) => (
									<div>
										<div class={styles.entry}>
											...
											>
												<Expander
													expanded={expandedPages().includes(
														index
													)}
												/>{" "}
												[{index * 100}...
